...
|     | 如果在进入单用户模式时仍需输入 root 密码，则表明控制台在/etc/ttys 中被标记为 insecure 。在这种情况下，必须从 FreeBSD 安装光盘引导，选择 Live CD 或 Shell 并在安装过程开始时执行上述命令。在这种情况下挂载具体分区，然后 chroot 到其中。例如，对于 ada0p1 上的系统，将 mount -urw / 替换为 mount /dev/ada0p1 /mnt; chroot /mnt 。 |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
